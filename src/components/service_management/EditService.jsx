...
const EditService = () => {
  ...
  // convert populated or raw id to raw id
  ...
  const fetchSubsubcategoriesByCategory = async (subCategoryId) => {
    if (!subCategoryId) return setSubSubCategories([]);
    const res = await getAxios().get(`/subsubcategories/subcategory/${subCategoryId}`);
    setSubSubCategories(res.data.data || []);
  };
  const fetchThemesBySubSubCategory = async (subSubCategoryId) => {
    if (!subSubCategoryId) return setThemes([]);
    const res = await getAxios().get(`/themes/subsubCategory/${subSubCategoryId}`);
  ...
  const fetchServiceDetails = async () => {
    // use the same base client you use elsewhere to avoid env mismatch
    const res = await getAxios().get(`/services/${serviceId}`);
    const s = res.data.data;

    setService(s.serviceName || "");
    setSelectedCategory(asId(s.categoryId));
    setSelectedSubCategory(asId(s.subCategoryId));
    setSelectedSubSubCategory(asId(s.subSubCategoryId));
    setSelectedTheme(asId(s.themeId));
    setPackageDetails(s.packageDetails || "");
    setRequiredDetails(s.requiredDetails || "");
    setCustomizedInputs(Array.isArray(s.customizedInputs) ? s.customizedInputs : []);
    setSelectedBalloonColors(Array.isArray(s.balloonColors) ? s.balloonColors : []);
    setOriginalPrice(s.originalPrice ?? "");
    setOfferPrice(s.offerPrice ?? "");
  ...
  // orchestrate initial load so options exist before value binding
  useEffect(() => {
    (async () => {
      try {
        setError("");
        await Promise.all([fetchCategories(), fetchBalloons()]);
        await fetchServiceDetails();
      } catch (e) {
        setError(e?.response?.data?.message || e.message || "Failed to load initial data");
      }
    })();
  }, []);

  // cascade loads (no resets here; we reset only on user change)
  useEffect(() => {
    if (selectedCategory) fetchSubcategoriesByCategory(selectedCategory);
  }, [selectedCategory]);
  useEffect(() => {
    if (selectedSubCategory) fetchSubsubcategoriesByCategory(selectedSubCategory);
  }, [selectedSubCategory]);
  useEffect(() => {
    if (selectedSubSubCategory) fetchThemesBySubSubCategory(selectedSubSubCategory);
  }, [selectedSubSubCategory]);
  ...
  const handleAddCustomizedInput = (e) => {
    e.preventDefault();
    const { label, inputType, maxValue } = customizedInput;
    if (!label || !inputType) return alert("Please fill all customized input fields.");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // send JSON (not FormData) since you only send URLs/strings
      const payload = {
        serviceName: service,
        categoryId: selectedCategory,
        subCategoryId: selectedSubCategory,
        subSubCategoryId: selectedSubSubCategory || null,
        ...
        originalPrice,
        offerPrice,
        balloonColors: selectedBalloonColors,
        images,
      ...
      await authAxios.put(`/services/update/${serviceId}`, payload); // localhost:5000 base should be in getAuthAxios
      alert("Service Updated Successfully");
      navigate("/service/service-list");
    } catch (error) {
      ...
      <div className="mx-auto bg-white shadow-lg rounded-lg p-6 lg:px-6 lg:py-2">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-3 gap-5">
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // reset children only on user change
                  setSelectedSubCategory("");
                  setSelectedSubSubCategory("");
                  setSelectedTheme("");
                }}
            ...
            {/* Sub-Subcategory */}
            <div className="relative">
              <label className="block text-gray-700 font-medium mb-1">Sub-Subcategory</label>
              <select
                value={selectedSubSubCategory}
                onChange={(e) => {
                  const v = e.target.value;
                  setSelectedSubSubCategory(v);
                  setSelectedTheme("");
                }}
                disabled={!selectedSubCategory}
                className="px-4 py-2 border border-gray-300 rounded-md w-full"
          ...
          <div className="mt-5">
            ...
            <div className="flex gap-4 flex-wrap">
              {images.map((url, index) => (
                <div key={index} className="relative">
                  <img src={url} alt={`Preview ${index}`} className="w-32 h-32 object-cover rounded-md" />
                  <button
                    className="absolute top-0 right-0 bg-red-600 text-white rounded-full p-0 text-sm p-1"
                    onClick={() => handleRemoveImage(index)}
                  >
          ...
          <div className="mt-5">
            <label className="block text-gray-700 font-medium mb-1">
              Package Details - Inclusion <span className="text-red-600">*</span>
            </label>
            <CKEditor editor={ClassicEditor} data={packageDetails} onChange={(_, ed) => setPackageDetails(ed.getData())} />
          </div>

          {/* Required Details */}
          <div className="mt-5">
            <label className="block text-gray-700 font-medium mb-1">Required Details</label>
            <CKEditor editor={ClassicEditor} data={requiredDetails} onChange={(_, ed) => setRequiredDetails(ed.getData())} />
          </div>

          {/* Customized Inputs */}
          <div className="mt-5">
            <h3 className="text-lg font-semibold mb-2">Customized Input Details</h3>
            ...
            <button type="button" onClick={handleAddCustomizedInput} className="p-2 px-4 bg-gray-600 text-white rounded-md flex items-center gap-1">
              <FiPlus /> Add
            </button>

            <ul className="list-disc pl-5 mt-4">
              {customizedInputs.map((input, index) => (
                <li key={index} className="flex items-center gap-2 mb-2 bg-gray-200 p-2 rounded-md">
                  <span className="flex-1">
                    <strong>{input.label}</strong> ({input.inputType}
                    {input.maxValue ? `, Max: ${input.maxValue}` : ""})
                  </span>
                  <button type="button" onClick={() => handleRemoveCustomizedInput(index)} className="text-red-600 hover:text-red-800">
                    <FiTrash2 />
                  </button>
                </li>
              ))}
          ...
          <div className="mt-4">
            ...
            <select
              onChange={(e) => {
                ...
                if (color && !selectedBalloonColors.includes(color)) {
                  setSelectedBalloonColors([...selectedBalloonColors, color]);
                }
              }}
              className="mt-4 px-4 py-2 border border-gray-300 rounded-md w-full"
            >
              ...
              {balloons.map((b) => (
                <option key={b._id} value={b.balloonColor}>
                  {b.balloonColor}
                </option>
              ))}
            </select>
            ...
            <div className="mt-4">
              ...
              <ul className="list-disc flex gap-4 flex-wrap">
                {selectedBalloonColors.map((color) => (
                  <li key={color} className="flex items-center gap-2 bg-gray-200 p-2 rounded-md">
                    <span className="flex-1">{color}</span>
                    <button
                      className="text-red-600 hover:text-red-800"
                      onClick={() => setSelectedBalloonColors(selectedBalloonColors.filter((c) => c !== color))}
                    >
                      <MdCancel />
                    </button>
                  </li>
          ...
          <div className="mb-6">
            <label className="font-semibold block mb-2">Caption</label>
            <CKEditor editor={ClassicEditor} data={caption} onChange={(_, ed) => setCaption(ed.getData())} />
          </div>
          ...
          <div className="mt-5">
            ...
            {faqs.map((faq, index) => (
              <div key={index} className="grid grid-cols-2 gap-4 mb-2">
                ...
                />
                <div className="col-span-2 text-right">
                  <button type="button" onClick={() => removeFaq(index)} className="text-red-500 hover:text-red-700 text-sm">
                    Remove FAQ
                  </button>
                </div>
              </div>
            ...
            <button type="button" onClick={addFaq} className="mt-2 text-blue-600 hover:text-blue-800 text-sm">
              + Add another FAQ
            </button>
          </div>
          ...
          {/* Actions */}
          <div className="flex justify-end gap-2 ">
            <button type="submit" className="rounded-lg bg-gray-700 px-4 py-2 text-white my-4">
              {loading ? "Updating..." : "Update"}
            </button>
            <button onClick={handleCancel} className="rounded-lg bg-red-700 px-4 py-2 text-white my-4">
              Cancel
            </button>
          </div>
        </form>
