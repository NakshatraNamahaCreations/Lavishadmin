...
const OrderDetail = () => {
  ...
  const isEventPast = (() => {
    const dateStr =
      orderDetails?.rescheduledEventDate || orderDetails?.eventDate;
    return dateStr ? dayjs(dateStr).isBefore(dayjs(), "day") : false;
  })();
  ...
  const handleStatusDropdownChange = (e) => {
    ...
    const hasDue =
      orderDetails.paymentPercentage === 50 &&
      orderDetails.paidAmount < orderDetails.grandTotal;

    // If marking as completed → and HALF payment → OPEN PAYMENT MODE MODAL
    if (newStatus === "completed" && hasDue) {
      setSelectedPaymentMode(""); // reset
  ...
  // UPDATED: Added extraPaymentMode parameter
  const handleStatusChange = async (newStatus, extraPaymentMode = null) => {
    ...
    try {
      ...
      // Add extraPaymentMode for completed status with HALF payment
      if (newStatus === "completed" && extraPaymentMode) {
        payload.extraPaymentMode = extraPaymentMode;
      }
      ...
      if (response.data.success) {
        ...
        setOrderDetails((prev) => ({
          ...prev,
          ...response.data.order, // Use the updated order from backend response
  ...
  const handleRescheduleOrder = async () => {
    ...
    if (rescheduleDate) payload.rescheduledDate = rescheduleDate;
  ...
  // Calculate billing amounts based on order schema
  const calculateBillingAmounts = () => {
    ...
    // For older orders where paymentStatus is "PAID" but paidAmount/dueAmount don't exist
    if (
      paymentStatus === "PAID" &&
    ...
    // For newer orders with paidAmount/dueAmount fields
    return {
      subTotal: subTotal || 0,
      couponDiscount: couponDiscount || 0,
      deliveryCharges: deliveryCharges || 0,
    ...
    <div className="max-w-6xl mx-auto p-8 bg-gradient-to-r from-gray-100 to-slate-200 shadow-xl rounded-lg">
      ...
      <div className="bg-white p-6 rounded shadow-sm my-10">
        ...
        <div className="divide-y divide-gray-200 text-lg">
          ...
          <div className="flex justify-between py-2">
            ...
            <span className="text-green-600">− ₹{billing.couponDiscount}</span>
          </div>
          <div className="flex justify-between py-2">
            <span>Delivery Charges</span>
            <span>₹{billing.deliveryCharges}</span>
          </div>
          <div className="flex justify-between py-2 font-medium">
            <span>Paid Amount</span>
            <span className="text-green-700">₹{billing.paidAmount}</span>
          </div>
          {billing.dueAmount > 0 && (
            <div className="flex justify-between py-2">
              <span>Due Amount</span>
              <span className="text-red-600">₹{billing.dueAmount}</span>
            </div>
          )}
          <div className="flex justify-between py-3 text-xl font-bold border-t mt-2 pt-3">
            <span>Grand Total</span>
            <span className="text-purple-700">₹{billing.grandTotal}</span>
          </div>
        </div>
      </div>
      ...
      {showPaymentModeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-[90%] max-w-md p-6 rounded-2xl shadow-xl border border-gray-200 animate-slideUp">
            <h2 className="text-2xl font-extrabold text-purple-800 text-center mb-3">
              Final Payment Required
            </h2>

            <p className="text-gray-600 text-center mb-6 leading-relaxed">
              The customer has a pending balance. Please select the mode of
            ...
            <div className="flex gap-4">
              <button
                onClick={() => {
                  ...
                  handleStatusChange("completed", "ONLINE");
                }}
                className="w-full bg-pink-600 hover:bg-pink-700 text-white py-3 rounded-xl text-lg font-semibold shadow-md transition-all"
              >
                ONLINE PAYMENT
              </button>
              ...
              <button
                onClick={() => {
                  setShowPaymentModeModal(false);
                  handleStatusChange("completed", "CASH");
                }}
                className="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl text-lg font-semibold shadow-md transition-all"
              >
                CASH PAYMENT
              </button>
            </div>
            ...
            <button
              ...
              className="mt-6 w-full py-2 bg-gray-200 hover:bg-gray-300 rounded-xl text-gray-700 font-medium transition-all"
            >
              Cancel
            </button>
          </div>
...
//   // Helper to check if event date is past
//   // const isEventPast = orderDetails?.eventDate || orderDetails?.eventDate
...
// const handleStatusChange = async (newStatus, paymentMode = null) => {
//     if (!orderDetails) return;
...
//       <p className="text-gray-600 text-center mb-6 leading-relaxed">
//         The customer has a pending balance.
//         Please select the mode of payment collected at the venue.
//       </p>
